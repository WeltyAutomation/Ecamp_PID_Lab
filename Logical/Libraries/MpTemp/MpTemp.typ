...
TYPE
	MpTempControllerConfigType : 	STRUCT  (*Configuration structure of the controller.*)
		...
		AmbientTemperature : REAL := 25.0; (*Ambient temperature. Unit[ï¿½C].*)
...
TYPE
	...
	MpTempTuningType : 	STRUCT  (*Tuning parameters.*)
		...
		SetPointHeat : REAL := 160.0; (*Set point temperature for heating. Unit: [ï¿½C].*)
		SetPointCool : REAL := 100.0; (*Set point temperature for cooling. Unit: [ï¿½C].*)
	END_STRUCT;
	...
	MpTempProfileParType : 	STRUCT  (*Profile parameters.*)
		SlewRate : REAL := 5.0; (*Limitation of the slope of the SetPointTemperature signal. Unit: [ï¿½C/s].*)
...
TYPE
	MpTempControllerInfoType : 	STRUCT  (*Controller information.*)
		SetTemperature : REAL; (*Smooth set temperature profile. Unit: [ï¿½C].*)
	...
	MpTempControlInfoType : 	STRUCT  (*PID controller information.*)
		ControlError : REAL; (*Control error. Unit: [ï¿½C].*)
		...
		ActualTemperatureFiltered : REAL; (*Filtered actual temperature. Unit: [ï¿½C].*)
		ActualTemperatureDerivative : REAL; (*Derivative of the actual temperature. Unit: [ï¿½C/s].*)
	...
	MpTempSimulationInfoType : 	STRUCT  (*Simulation information.*)
		...
		ActualTemperature : REAL; (*Actual temperature. Unit: [ï¿½C].*)
	END_STRUCT;
